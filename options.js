...
// تعني «لا شارة» لقنوات الشارة كلِّها** — ⭐ **والرفضُ في `showBadge` يقع قبل أن
// تُعرَف القناة، فالمنعُ بالبناء لا بالتوافق** (#75، 2026-08-08). ⛔ **ولا يُكتب
// هنا عددُها**: عددٌ بيدٍ يتخلّف، وقد تخلّف هذا النصُّ بعينه منذ #134 حين قال
// «الشارتان» والمستهلكون ثلاثة. فضابطٌ يُضغط ولا يفعل شيئاً **انحدارٌ** (#24) —
...
function syncSpeedBadgeRow(vol) {
  ...
  if (body && c) {
    body.textContent = off
      ? "معطّلة الآن: «كم تبقى الشارة ظاهرة» صفر، والشاراتُ كلُّها تتشارك المدّة نفسها. ارفعها فوق الصفر لتعمل."
      : c.help;
  }
}
