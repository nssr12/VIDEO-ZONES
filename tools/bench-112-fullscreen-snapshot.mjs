...
// ── ⭐⭐ م13 — **رُفعت 2026-08-08 بشاهدٍ يُسقط ضماناتها معاً** ────────────────
// ⛔ **وكانت خضراءَ بلا شاهد حمرة فلم تُرفع**، ⭐ **والعلّةُ سُمّيت (قرار المالك):
// وعدٌ مضمونٌ مرّتين لا يُحمَّر بإزالة إحدى ضمانتيه** — **وهي أختُ `م5` و`م22`.**
// **والضمانتان القائمتان هنا مقيستان**: `idlePointerHeld` · **و`pointerInsideEl`
// (#95)** — **موضعُ الضغط `y≈505` والشريطُ عند `541` والهامشُ `40`** ⇒ **داخلَ
// المنطقة**. ⚠️ **والتركيزُ ليس منها**: الضغطةُ على الصورة لا على الشريط.
// ✅ **والمقيس بإسقاطهما معاً: `شريط=false · صنفُنا=true`** ⇒ **الخطوةُ تكشف.**
// ⛔ **وحدُّها مكتوبٌ فيها: تُحمَّر بإسقاط ضماناتها معاً لا بإسقاط إحداها** —
// **وهو إنتاجُ حالٍ سالبة لا عزلُ سبب، ولا يُقرأ عزلاً** (وليس نقضاً لقرار 42:
// **سؤالُ الشاهد «أتكشف الخطوةُ غيابَ الأثر؟» لا «أيُّ سببٍ فعلها»**).
console.log("\n[2] م13 — مسكُ الزرّ بلا حركة في ملء الشاشة: الشريطُ باقٍ تحت اليد");
check("[2] ⭐⭐ (م13) لا يُخفى تحت يدٍ ماسكة — تُحمَّر بإسقاط ضمانتيها معاً",
  r.م13?.شريط === true && r.م13?.صنفُنا === false, r.م13);
