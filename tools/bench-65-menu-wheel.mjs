...
// ⭐ **استوفى معايير البوّابة الأربعة بالقياس ولم يدخلها — والسببُ مقيسٌ لا مُقدَّر:**
//   محليٌّ · حتميّ · رخيصٌ (16 ثانية) · **وله حكمٌ يُحمّر منذ اليوم** (`exit 1`
//   على عطب #65 مزروعاً، `exit 0` سليماً). ⇒ **ثمّ قِيس أن
//   `tools/test-scroll-layer.js` يُحمّر على العطب نفسِه — في المجموعة، بلا
//   متصفّح، في أقلّ من ثانية** (أربعةُ تأكيدات على البناء المزروع نفسِه).
//   ⇒ ⭐ **فالسؤال الأخير ليس «أيستطيع أن يُحمّر؟» بل «أيُحمّر على ما لا يُحمّر
//   عليه غيرُه؟»** — **وعضوٌ يكرّر تغطيةَ عضوٍ أرخصَ منه كلفةٌ بلا مقابل** (قرار 90).
//   ⚠️ **وحدُّه: قِيس عطبٌ واحد**، ولم يُقس عطبٌ يُمسكه هذا وحده.
//   ومسارُه الحيّ خلف `--hosts` وحده.
//
...
// ── ⭐ الحكمُ على المنتَج — **يُضاف ولا يحلّ محلّ الامتناع** (قرار المالك 2026-08-04)
// **العلّة:** كان هذا الرِكاز **يطبع «❌ مختطَفة» و«❌ انكسرت» ويخرج بصفر** — فهو
// يرى العطبَ ولا يقوله لمن يُشغّله. ⇒ **مِقياسٌ لا حارس**، وهو ما أخرجه من
// ترشيح البوّابة أصلاً (المعيار الرابع: **وله حكمٌ يستطيع أن يُحمّر**).
// ⛔ **وشرطُ الإضافة بنصّه: ما يستطيع أن يحكم فيه يحكم، وما لا يستطيع يمتنع** —
// **ولا يُقلب امتناعٌ إلى أحمرَ ليصير «حارساً»**، فذاك يشتري صفةَ الحارس بثمن
// الصدق. ⇒ **فالامتناع يبقى خارج الحكم** ولا يُعدّ سقوطاً.
// ⚠️ **وتمييزُ «امتنع» عن «أخضر» برمز الخروج هو عملُ #105 المؤجَّل** — فيُقال هنا
// بالسطر حتى يُنفَّذ هناك بالرمز، **ولا يُدَّعى أنه مُميَّزٌ اليوم**.
{
  if (!pos.ok) {
    console.log(`\n⚪ **امتنع — لا حكم**: الذراع الموجبة لم تُنتَج. ${pos.note || ""}`);
  } else {
    const scrolled = pos.menuAfter.scrollTop > pos.menuBefore.scrollTop;
    const baseWorks = pos.videoAfter.volume !== pos.videoBefore.volume;
    const bad = [];
    if (!scrolled) bad.push("القائمة مختطَفة — عجلتُنا ابتلعت تمريرها (#65)");
    if (!baseWorks) bad.push("الميزة الأساسية انكسرت — العجلة فوق الفيديو لا تُغيّر المستوى");
    console.log(`\n${bad.length ? "❌" : "✅"} الحكم: ${bad.length ? bad.join(" · ") : "القائمة تُمرَّر، والعجلة فوق الفيديو تعمل"}`);
