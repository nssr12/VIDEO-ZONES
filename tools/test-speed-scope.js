...
//
// ⭐ **السؤال الذي يجيبه (بلغة المستخدم):** *«هل يظهر زرّ السرعة على مشغّلٍ حقيقيّ وحده، لا على معاينةٍ في الصفحة الرئيسية؟»*
//
...
//
// ── ⛔⭐ حالُ البنيات الخمس بعد #116 — **مقيسةٌ 2026-08-06، وصفرُ حذف** ──────
//
// **طُلب حذفُ ثلاثٍ منها بوصفها «مساراتٍ لا تقع بعد تضييق الزرّين إلى يوتيوب»**
// (لا حارسَ يحرس ما لا يُستعمل — #94 بنصّه). ⇒ ⛔ **ولم تُحذف واحدة، وهذي
// أسبابُها الثلاثة، كلٌّ بسببه هو لا بسبب مجموعته:**
//
//  · **تويتش/`d.tube` (التثبيت)** — ⛔ **حيٌّ، وإدراجُه في الثلاث كان خطأً منّي**:
//    هو يفحص **تثبيتَ الحكم على أوّل نجاح**، **ويوتيوب يُخفي شريطَه بعد ~3 ثوانٍ**
//    (وهي مقدّمةُ #70 المقيسة) ⇒ **بلا تثبيتٍ يختفي الزرُّ مع الشريط على يوتيوب**.
//    ⭐ **وقد قلتُ ذلك بنفسي في تحليل #116 ثمّ أدرجتُه في القائمة المقابلة** —
//    **وتناقضٌ داخل عرضٍ واحد يُبنى عليه قرار، فيُسجَّل ولا يُمرّر.**
//
//  · **فيميو (أبعدُ سلفٍ لا أقربه)** و **فيديو خام (لا نطاق)** — ⚠️ **مسارُهما
//    لم يُبلَغ في القياس، ولم يُثبَت أنه لا يُبلَغ.** **المقيس على المضيف الحيّ:**
//    `watch` · `nocookie/embed` · **والرئيسية** ⇒ **`#movie_player` في ثلاثتها**،
//    **و`closest(KNOWN_PLAYER_WRAPPER…)` يفوز أوّلاً فلا يُبلَغ مسارُ النقاط ولا
//    فرعُ «لا نطاق».** ⇒ ⛔ **وثلاثةُ أسطحٍ عيّنةٌ لا مجال**، **وحالُ المعاينة
//    الحيّة لم تُنتَج أصلاً** (بطاقةٌ بمستطيل صفريّ في المتصفّح المقطوع الرأس).
//    ⇒ ⭐ **والشرط كان «يُحذف بإثباتٍ لا بترجيح» — ولا إثبات، فلا حذف.**
//
// ⇒ ⭐⭐ **والقاعدة التي خرجت: «صار غيرَ قابلٍ للوصول» دعوى تُقاس كما يُقاس أيُّ
// خبرٍ عن سلوك** — **ونفيُ الوصول أثقلُ من إثباته: إثباتُه عيّنةٌ واحدة، ونفيُه
// يشترط استيفاءَ المجال.** ⛔ **فما لم يُستوفَ لا يُحذف، ويُكتب أنه لم يُستوفَ.**
//
