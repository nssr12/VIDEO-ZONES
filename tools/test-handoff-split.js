...
//
// ⚠️ **الثالث هو ما يمنع عودة 2724 سطراً (قرار المالك):** `HANDOFF` بلغ ما بلغ
// **بالتراكم لا بقرار**. **وسقفُ أسطرٍ رقمٌ اعتباطيّ يُتحايَل عليه بالاختصار**،
...
console.log("\n[3] بصمةُ النقل — نقلٌ لا حذف");
{
  ...
  const lost = man.lines.filter((h) => !have.has(h));
  check(`كلُّ ما نُقل باقٍ (${man.count} سطراً)`, lost.length === 0, `${lost.length} ضاع`);

  // شاهدٌ على الحارس: بصمةٌ مُفتعَلة **يجب أن تُقرأ ضائعة**
  check("والحارس يرى الضياع (شاهد مُفتعَل)", !have.has("0000deadbeef"));
}
