...
console.log("\n[3] ⭐ الملكية بسمةٍ على العنصر، وتُحسم في الكومة");
{
  ...
  // ⛔⭐ **والتعليقاتُ تُنزع قبل العدّ** — **وإلا عدَّ الحارسُ شرحاً يذكر الوسمَ
  // عنصراً يحمله**: وقع فعلاً 2026-09-05 (#142) — **تعليقٌ في ورقة الأنماط يشرح
  // «pointer-events:auto على الشريط وحده» ويذكر الوسم** ⇒ **فطبع العددَ 6 والعناصرُ 5.**
  // ⇒ **وهي رابعةُ «مطابقةٌ أوسعُ من سؤالها» (قرار 93)، والعلاجُ نفسُه الذي في
  // `tools/test-projection-keys.js`: السؤالُ «أيحمله عنصر؟» لا «أيُذكر اسمُه؟».**
  const SRC_NC = SRC.split("\n").filter((l) => !/^\s*(\/\/|\*|\/\*)/.test(l)).join("\n");
  const n = (SRC_NC.match(/data-vz-owns/g) || []).length;
  // ⚠️ **العدد يُحدَّث بوعي ولا يُرفع ليمرّ:** صار ثلاثةً بـ#108 —
  // الزرّ (#72) · وزرّ الفلاتر · ولوحتُها. **ورابعٌ يُحمّر فيُسأل عنه.**
  // ⭐ **وقد احمرَّ ورابعُه سُئل عنه وأُجيب (2026-08-08، #134): زرُّ نسخ الرابط**
  // — **يملك عجلتَه ونقرتَه كأخويه، فيحمل العلامة بالضرورة.**
  // ⛔ **ولم يُشتقَّ العددُ عمداً**: قُدِّم اشتقاقُه فأُبقي مكتوباً بيد **لأن
  // وقفتَه البشريّة هي غرضُه** — **واشتقاقُه يُلغيه** (وأثرُه أن كلَّ زرٍّ خامس
  // يدفع سطراً هنا، **وذاك ثمنُ الوقفة لا عيبُها**). **والقرارُ للمالك إن رآه غالياً.**
  // ⭐ **وخامسٌ سُئل عنه وأُجيب (2026-09-05، #142): شريطُ التقدّم لمضيفٍ بلا أدوات**
  // — **يملك سحبَه ونقرتَه، فيحمل العلامة بالضرورة كأخواته.**
  check("[3] وخمسةُ عناصرَ تحملها اليوم (#72 · #108 زرّاً ولوحةً · #134 · #142)", n === 5, `العدد ${n}`);
...
console.log("\n[7] ⭐ #76: يضمن عنصره بنفسه، ولا يرث بناءً من مسار المربّعات");
{
  ...
  // ⛔⭐⭐ **صار يُشتقّ 2026-08-08 (#134، تصنيفُ المالك):** كان عدّاً مكتوباً بيد
  // (**ثمانية**) **وهو ينمو مع كلّ مستهلكٍ بالتصميم** ⇒ **فثمنُه يُدفع عند كلّ
  // زرٍّ رابعٍ وخامس بلا مقابل** (قرار 34).
  // ⭐ **والخاصّيةُ هي المقصود لا العدد** (#76): **كلُّ مُظهِرٍ يضمن عنصرَه بنفسه،
  // فلا يرث مستهلكٌ بناءً من مسارٍ قد لا يمرّ.** ⇒ **ومُظهِرٌ جديد لا يضمن عنصرَه
  // يُحمّر، وهو ما بُني الشرطُ له — والعددُ لم يكن يمسكه أصلاً.**
  {
    const showers = [...CODE.matchAll(/function (set\w*BtnShown)\(/g)].map((m) => m[1]);
    check("[7] ⭐ ومُظهِرو الأزرار معلومون (مشتقّون)", showers.length >= 3, showers);
    for (const fn of showers) {
      const b = body(`function ${fn}(on)`) || "";
      check(`[7] و\`${fn}\` تضمن عنصرَها (#76)`, /ensureVideoOverlay\(/.test(b), fn);
    }
  }
}
...
{
  // ⭐ **الوسمُ صار يُبنى بالراسم الواحد** (#131) — **فيُقاس النداءُ والحقل،
  // لا وسمٌ حرفيّ**: وسمٌ مكتوبٌ بيد هو بعينه ما مُنع، **فشرطٌ يشترطه يُعيد ما أُزيل.**
  check("[9] الزرّ يحمل أيقونةً من الراسم الواحد",
    /vzSvg\(VZ_OWN_ICONS\[\"speed\"\], \{ cls: \"vzSpeedIcon\" \}\)/.test(SRC));
  check("[9] ومقاسُها حقلٌ في السجلّ المشحون يطابق المقيس (24)",
    /\"speed\":\s*\{\s*viewBox:\s*\"0 0 24 24\"/.test(SRC));
