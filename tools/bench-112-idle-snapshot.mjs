...
// **المرفوعُ عن المالك: م1 · م2 · م3 · م4 · م5 · م22.**
//
...
// ⇒ ⭐⭐ **وهي أختُ `م5` و`م13`، والشكلُ الجامعُ سُمّي 2026-08-08 (قرار المالك):**
// **وعدٌ مضمونٌ مرّتين لا يُحمَّر بإزالة إحدى ضمانتيه.** ⇒ **والشاهدُ الذي يُزيل
// واحدةً يبقى أخضرَ بحقّ** — **وليس ذاك عطباً في المنتَج بل خاصّيةً فيه**،
// ⭐ **والخاصّيةُ التي تحمي المستخدم هي بعينها التي تمنع عزلَ السبب.**
//
// ✅ **ورُفعت 2026-08-08 بعد قياسٍ استردّها: تُسقَط الضمانتان معاً في بناءٍ واحد**
// (الزرُّ يخرج من الشريط · و`onIdle` لا يُخفي) ⇒ **`زرُّ_السرعة = true` تحت شريطٍ
// مخفيّ — وهي الحالُ السالبةُ بعينها.**
// ⛔⭐⭐ **وحدُّ هذا الشرط يُكتب فيه ولا يُترك للقارئ: يُحمَّر بإسقاط ضمانتيه معاً
// لا بإسقاط إحداهما.** ⚠️ **وليس هذا خلطاً للمتغيّرات ولا نقضاً لقرار 42:**
// **سؤالُ الشاهد «أتكشف الخطوةُ غيابَ الأثر أصلاً؟» لا «أيُّ سببٍ فعلها»** ⇒
// **فإنتاجُ الحال السالبة يقتضي إسقاطَ كلّ ما يكفي لوقوعها.**
// ⇒ **فهو إنتاجُ حالٍ سالبة لا عزلُ سبب، ولا يُقرأ عزلاً.**
console.log("\n[3] م22 — وزرُّ السرعة يختفي مع الشريط");
check("[3] ⭐⭐ (م22) زرُّ السرعة غيرُ مرئيٍّ في الحال المُخفاة — تُحمَّر بإسقاط ضمانتيها معاً",
  r.م22?.زرُّ_السرعة === false,
  { قِيس: r.م22?.زرُّ_السرعة,
    ملحوظة: r.م22?.زرُّ_السرعة === null ? "لا زرَّ أصلاً — حالٌ أخرى لا عطب" : "ظاهرٌ تحت شريطٍ مخفيّ" });
...
// ⇒ ⭐⭐ **فأخضرُها يقول «لم يُخفَ» ولا يقول «لأنّا امتنعنا»** (قرار 42 · 48).
...
//
// ✅ **ورُفعت 2026-08-08 بقياسٍ استردّها: تُسقَط الثلاثُ معاً في بناءٍ واحد** ⇒
// **`شريط=false · صنفُنا=true` — الشريطُ اختفى تحت اليد**، فالخطوةُ تكشف.
// ⛔⭐⭐ **وحدُّها مكتوبٌ فيها: تُحمَّر بإسقاط أسبابها الثلاثة معاً لا بإسقاط
// أحدها** — **وقد جُرّب واحدٌ ثمّ اثنان فلم يُحمّرا** (مقيسٌ لا مُقدَّر).
// ⚠️ **وثالثُها (`pointerInsideEl`) يُسقَط بسطرٍ واحد لكنّه قاعدةُ المحرّك العامّة
// (#95) لا قاعدةَ م5** ⇒ **فالبناءُ يختلف لكلّ مستهلك، ويُقال ولا يُدّعى عزلٌ.**
// ⇒ **سؤالُ الشاهد «أتكشف الخطوةُ غيابَ الأثر؟» لا «أيُّ سببٍ فعلها».**
console.log("\n[5] م5 — مسكُ شريط التقدّم بلا حركة: يبقى تحت اليد");
check("[5] شريطُ التقدّم في المنظور ليُمسك", r.م5موضع?.داخلَ_المنظور === true, r.م5موضع);
check("[5] ⭐⭐ (م5) لا يُخفى تحت يدٍ ماسكة — تُحمَّر بإسقاط أسبابها الثلاثة معاً",
  r.م5?.شريط === true && r.م5?.صنفُنا === false, r.م5);
...
console.log(`  ⚪ والتركيز بعد الضغط: ${JSON.stringify(r.م5تركيز)}` +
  ...
  `  ⇒ **الامتناعُ ينفكّ، ويبقى الشريطُ بامتناعٍ ثانٍ مُعلَن — مكتوبٌ في الكود لا عطب**`);
