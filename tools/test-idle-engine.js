...
//
// ⭐ **السؤال الذي يجيبه (بلغة المستخدم):** *«هل يختفي ما يجب أن يختفي بعد أن أرفع يدي عن الفأرة، ويعود بحركتها؟»*
//
...
console.log("\n[4] ⭐ المصيدة (٢): «النشاط يُقاس عند الإدخال لا عند أثره»");
{
  ...
  // الستّة كانت — المؤشّر (`noteIdleFromPointerEvent`) · ملء الشاشة ·
  ...
  // ⭐ **وثلاثةٌ زِيدت 2026-09-05 (#142) وسُئلت بالسؤال نفسِه فأُجيبت: الضغطُ على
  // شريط التقدّم · وسحبُه · ورفعُ الإصبع عنه** — **وثلاثتُها إدخالٌ مباشرٌ على
  // عنصرٍ نملكه، لا أثرَ إدخال**: ⛔ **ولولا السحبُ منها لاختفى الشريطُ تحت
  // الإصبع بانقضاء المهلة وهو يُسحَب.**
  const marks = (CODE.match(/markIdleActivity\(\)/g) || []).length;
  // ⭐ **وعاشرٌ سُئل وأُجيب (2026-09-06، #142د): نقرةُ زرّ ملء الشاشة في شريطنا**
  // — **إدخالٌ مباشرٌ على عنصرٍ نملكه، لا أثرَ إدخال** (وهي نقرةُ زرّ #72 نفسُها
  // من بابٍ ثانٍ). ⛔ **ولولاها لاختفى الشريطُ لحظةَ الضغط عليه.**
  // ⭐ **وحاديَ عشرَ سُئل وأُجيب (2026-09-06، #142هـ): نقرةُ زرّي التشغيل والصوت**
  // — **موضعٌ واحد يخدم زرّين** (`hostBarActionClick`)، **فالعددُ زاد واحداً لا اثنين**
  // ⇒ ⭐ **وهو ما تشتهيه هذي القاعدة: موضعٌ يُسأل عنه مرّةً لا موضعان يتباعدان.**
  check("[4] ومواضع نداء النشاط معدودة ومقصودة (١١)", marks === 11, `العدد ${marks}`);
...
console.log("\n[11] ⭐ كل مستهلكٍ يُعلن معنى إطفائه، والمحرّك ينقل لا يقرّر");
{
  ...
  // ⭐ **#107 — الفرعُ انقسم ولم تنقص تغطيتُه:** «ممتنع» و«المؤشّر على الهدف»
  // **يُسمّيان الآن** بدل أن يُدمجا في «نشط» واحدة، **والمستهلك يقرأ السبب**.
  // **ويُشترط الثلاثة معاً**: امتناعٌ ⇒ `suspended` · مؤشّرٌ ⇒ `pointer` ·
  // ونشاطٌ ⇒ `active` — **فحذفُ أيّها يُحمّر.**
  check("[11] و«ممتنع» يبقى كالنشط بسببه المسمّى — وهو غيرُ «مُطفأ»",
    /c\.suspended\?\.\(\)\)\s*\{\s*c\.onActive\("suspended"\)/.test(body), body.slice(0, 320));
  check("[11] و«المؤشّر على الهدف» كذلك، بسببه هو",
    /pointerInsideEl\(c\.target\?\.\(\), c\.nearPad\?\.\(\) \?\? 0\)\)\s*\{\s*c\.onActive\("pointer"\)/.test(body),
    body.slice(0, 320));
  check("[11] و«نشاط» يُسمّى كذلك — فلا يُخلط بالسببين",
    /idleState === "idle"\) c\.onIdle\(\); else c\.onActive\("active"\)/.test(body), body.slice(0, 320));
  // ⭐ **#106 — والهامشُ من إعلان المستهلك لا من حكم المحرّك.** محرّكٌ يكتب الرقم
  // بنفسه **يفرض سياسةً على مستهلكيه كلِّهم** — وهو عينُ الحدّ المعماريّ الذي
  // بُني له هذا المحرّك (**يُصدر حالاً ولا يُصدر أمراً**).
  check("[11] ⭐ ولا رقمَ هامشٍ في المحرّك — يسأل المستهلك ولا يقرّر",
    !/IDLE_NEAR_PAD_PX/.test(body) && !/pointerInsideEl\([^)]*,\s*\d+\s*\)/.test(body), body.slice(0, 320));
