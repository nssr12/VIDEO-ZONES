...
const VZ_UI_TIMING = [
  ...
  { id: "volumeDuration", kind: "range", min: 0, max: 3000, step: 100, unit: "ms",
    مدى: "عامّ", أين: "طبقتُنا فوق أي فيديو.",
    label: "كم تبقى الشارة ظاهرة",
    help: "تسري على كلّ شارةٍ تظهر فوق الفيديو، لا على رقم الصوت وحدَه. ⚠️ والصفر يعني: لا تظهر أيّ شارة — ولذلك يُعطَّل مفتاح شارة السرعة عنده بسببٍ مكتوب." },
  ...
  { id: "speedBadgeEnabled", kind: "toggle", مدى: "عامّ", أين: "طبقتُنا فوق أي فيديو.",
    ...
    help: "تظهر في الزاوية المقابلة لرقم الصوت، وتأخذ لون الشارات وحجمها ومدّتها. ⚠️ ولا يومض لها يوتيوب — قِيس أن الكتابة المباشرة لا تُنتج وميضاً." },
  { id: "hostBarEnabled", kind: "toggle", مدى: "مضيف",
    أين: "المواقعُ التي تضيفها أنت في القائمة أسفلَ هذا القسم — لا غيرُها، ولا يوتيوب بحال.",
    label: "أظهِر شريطَ تقدّمٍ على المواقع التي أختارها",
    help: "مواقعُ مثل تيك توك وإنستقرام تعرض الفيديو بلا شريط تقدّمٍ ولا أزرار، فلا سبيلَ إلى التنقّل فيه. وهذا يرسم شريطاً من عندنا فوق الفيديو: يظهر بحركة الفأرة ويختفي بالسكون، ويُسحَب للتنقّل، وفيه أزرارُ التشغيل والصوت وملء الشاشة. ⭐ والمواقعُ تُضاف في القائمة أسفلَه (أو بزرّ ▬ في نافذة الإضافة وأنت على الموقع) — فقائمةٌ فارغةٌ تعني أنه لا يُرسم في أيّ مكان. ⛔ ولا يُرسم على يوتيوب بحال. ⚠️ وفي موقعٍ أضفتَه وله شريطُه، يُرسم شريطُنا فوق شريطه — أنت طلبتَه فيه. ⛔ ولا يُرسم على فيديو يعرض ضوابطَ المتصفّح نفسِها: فيها التنقّلُ سلفاً. ⛔ ولا يعمل في البثّ المباشر: مدّةٌ غيرُ منتهية لا موضعَ فيها يُرسم." },
...
const VZ_BAR_BUTTONS = [
  // ⛔⭐⭐ **والأيقونةُ تُعلَن هنا 2026-08-08 (#134، تحقّقُ المالك)** — **وكان
  // المحرِّرُ يختارها بثلاثيّةٍ لا ببحث**: `it.id === "speed" ? "speed" : "filter-h"`
  // ⇒ **فالثالثُ أخذ رسمَ جاره صامتاً.** ⭐ **وسقوطٌ صامتٌ يُعيد السابقَ أخطرُ من
  // سقوطٍ يُظهر فراغاً: هذا يُقرأ صواباً والآخرُ يُسأل عنه.**
  { id: "speed",  label: "زرّ السرعة", icon: "speed",
  ...
  { id: "filter", label: "زرّ فلاتر الصورة", icon: "filter-h",
  ...
  { id: "copy", label: "زرّ نسخ رابط الفيديو", icon: "copy-link",
    help: "ينسخ رابطاً مجرّداً للفيديو (يوتيوب ومعرّفُ الفيديو فقط)، ويؤكّد النسخ بشارةٍ قصيرة. ⛔ ولا وقتَ ولا قائمةَ تشغيل ولا شيءَ سواهما — ⛔ ~~وكان يُبقي القائمةَ~~ فبُدِّل بقرار المالك بعد أن جرّب. ⚠️ وفي Shorts (إن أطفأتَ «تحويل Shorts إلى المشغّل العادي») لا معرّفَ في الرابط فلا نسخ، ويقول ذلك. ⚠️ ومدّةُ التأكيد من «كم تبقى الشارة ظاهرة» — فإنزالُها إلى صفرٍ يُطفئ التأكيد، كشارة السرعة تماماً." }
];
...
function vzUiBuildBarEditor(doc, root, opts = {}) {
  ...
  function render() {
    ...
    for (const it of l) {
      ...
      // ⛔ **بحثٌ بالمعرّف المُعلَن، ومجهولٌ ⇒ لا رسمَ بل الاسم** (الفرعُ أدناه)
      // — **فالرابعُ غداً يُظهر اسمَه لا رسمَ جاره، ويُسأل عنه.**
      const ourIco = VZ_SIM_OUR_ICONS[meta(it.id).icon];
      if (ourIco) {
